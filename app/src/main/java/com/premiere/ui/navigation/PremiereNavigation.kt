package com.premiere.ui.navigation
                ...
                viewModel.effect.collect { effect ->
                    when (effect) {
                        ...
                        is MoviesListContract.Effect.NavigateToFilters -> {
                            val encodedFilters = Uri.encode(Json.encodeToString(effect.filters))
        ...
        composable(
            ...
            arguments = listOf(
                navArgument(FILTERS_ARG) {
                    ...
                    defaultValue = Uri.encode(Json.encodeToString(MovieFilters()))
                }
            )
        ) { backStackEntry ->
            val encodedFilters = backStackEntry.arguments?.getString(FILTERS_ARG).orEmpty()
            val initialFilters = Json.decodeFromString<MovieFilters>(Uri.decode(encodedFilters))
